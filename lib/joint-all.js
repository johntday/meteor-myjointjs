...
jQuery.ready.promise = function( obj ) {
	if ( !readyList ) {
		...
		if ( document.readyState === "complete" ) {
			...
			Meteor.setTimeout( jQuery.ready );
...
jQuery.fn.extend({
	...
	delay: function( time, type ) {
		...
		return this.queue( type, function( next, hooks ) {
			var timeout = Meteor.setTimeout( next, time );
			hooks.stop = function() {
	...
	ajax: function( url, options ) {
		...
		if ( !transport ) {
			...
			if ( s.async && s.timeout > 0 ) {
				timeoutTimer = Meteor.setTimeout(function() {
					jqXHR.abort("timeout");
				}, s.timeout );
			}
...
jQuery.fx.start = function() {
	if ( !timerId ) {
		timerId = Meteor.setInterval( jQuery.fx.tick, jQuery.fx.interval );
	}
};
  ...
  _.extend(History.prototype, Events, {
    ...
    start: function(options) {
      ...
      if (this._hasPushState) {
        ...
        this._checkUrlInterval = Meteor.setInterval(this.checkUrl, this.interval);
      }
...
//    format: {},
	...
	nextFrame:(function() {
		...
		raf = function(callback) {
		    ...
		    var id = Meteor.setTimeout(function() { callback(currTime + timeToCall); }, timeToCall);
